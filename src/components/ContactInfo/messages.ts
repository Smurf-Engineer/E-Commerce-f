/**
 * ContactInfo - messages
 */

import { defineMessages } from 'react-intl'

export default defineMessages({
  title: {
    id: 'components.footer.contactInfo.title',
    defaultMessage: 'CONTACT INFO'
  },
  company: {
    id: 'components.footer.contactInfo.company',
    defaultMessage: 'Rider Sportsfashion LLC (dba JAKROO)'
  },
  companyCA: {
    id: 'components.footer.contactInfo.companyCA',
    defaultMessage: 'Jakroo Canada Inc.'
  },
  companyAU: {
    id: 'components.footer.contactInfo.companyAU',
    defaultMessage: 'PromoVelo Pty Ltd.'
  },
  subtitleAU: {
    id: 'components.footer.contactInfo.subtitleAU',
    defaultMessage: 'ABN/ACN: 66 619 807 224'
  },
  street: {
    id: 'components.footer.contactInfo.street',
    defaultMessage: '5906 Stoneridge Mall Road.'
  },
  streetCA: {
    id: 'components.footer.contactInfo.streetCA',
    defaultMessage: '2323 Quebec St #222.'
  },
  streetAU: {
    id: 'components.footer.contactInfo.streetAU',
    defaultMessage: '10 Ashbourne Tce Biggera Waters<br/>Gold Coast'
  },
  suburb: {
    id: 'components.footer.contactInfo.suburb',
    defaultMessage: ' Pleasanton, CA 94588'
  },
  suburbCA: {
    id: 'components.footer.contactInfo.suburbCA',
    defaultMessage: ' Vancouver, BC, V5T 4S7'
  },
  suburbAU: {
    id: 'components.footer.contactInfo.suburbAU',
    defaultMessage: ' Australia, QLD, 4216'
  },
  phone: {
    id: 'components.footer.contactInfo.phone',
    defaultMessage: '1.800.485.7067'
  },
  phoneCA: {
    id: 'components.footer.contactInfo.phoneCA',
    defaultMessage: '877.252.5766'
  },
  phoneAU: {
    id: 'components.footer.contactInfo.phoneAU',
    defaultMessage: 'Tel: 0422 246 192'
  },
  email: {
    id: 'components.footer.contactInfo.email',
    defaultMessage: 'designlab@jakroousa.com'
  },
  emailCA: {
    id: 'components.footer.contactInfo.emailCA',
    defaultMessage: 'info@jakroo.ca'
  },
  emailAU: {
    id: 'components.footer.contactInfo.emailAU',
    defaultMessage: 'aust@jakroousa.com'
  }
})

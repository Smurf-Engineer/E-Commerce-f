/**
 * ProDesign Types - Created by eduardoquintero on 19/09/19.
 */
const namespace = 'src/ProDesign'

// Tabs Key
export const UPLOAD = 'upload'
export const COLOR = 'color'

export const ON_TAB_CLICK = `${namespace}/ON_TAB_CLICK`
export const SET_SEARCH_PRODUCT = `${namespace}/SET_SEARCH_PRODUCT`
export const SET_PRODUCT_CODE = `${namespace}/SET_PRODUCT_CODE`
export const UPLOAD_FILE_ACTION_SUCCESS = `${namespace}/UPLOAD_FILE_ACTION_SUCCESS`
export const SET_UPLOADING_FILE_ACTION = `${namespace}/SET_UPLOADING_FILE_ACTION`
export const GO_TO_COLOR_SECTION = `${namespace}/GO_TO_COLOR_SECTION`
export const SET_STITCHING_COLOR_ACTION = `${namespace}/SET_STITCHING_COLOR_ACTION`
export const SET_COLOR_ACTION = `${namespace}/SET_COLOR_ACTION`
export const SET_SELECTED_USER = `${namespace}/SET_SELECTED_USER`
export const SET_INPUT_VALUE = `${namespace}/SET_INPUT_VALUE`
export const OPEN_MODAL = `${namespace}/OPEN_MODAL`
export const SET_SAVING_DESIGN = `${namespace}/SET_SAVING_DESIGN`
export const SAVE_DESIGN_SUCCESS = `${namespace}/SAVE_DESIGN_SUCCESS`
export const SET_USER_TO_SEARCH = `${namespace}/SET_USER_TO_SEARCH`

// Color Tabs
export const MAIN_SECTION = 0
export const STITCHING_SECTION = 1

...
export class DesignLabLanding extends React.Component<Props, StateProps> {
  state = {
  ...
  goToDesignLab = () => {
    const { selectedProduct = -1 } = this.state
    if (selectedProduct !== -1) {
      const { history } = this.props
      history.push(`/design-center?id=${selectedProduct}`)
    }
  }
  ...
  render() {
    ...
    const filtersGraph = filtersArray && filtersArray.length ?
      filtersArray.filter((itemOption) => 
        itemOption.name === 'gender' || 
        itemOption.name === 'sport' || 
        itemOption.name === 'category') : []
    let sortByLabel = ''
                    ...
                    <Container>
                      <ResultsColumn>
                        <SelectDiv>
                          ...
                          <SubtitleSelect>
                            Use the filters to browse sport and product categories
                          </SubtitleSelect>
                          <SelectDesign onClick={this.goToDesignLab} disabled={selectedProduct === -1}>
                            <DesignIcon type="skin" />
                            Start Design
                          </SelectDesign>
                        </SelectDiv>
                        <FiltersTitle
                        ...
                        <ProductsThumbnailList
                          ...
                          productToDesign={this.productToDesign}
...
const mapStateToProps = (state: any) => {
  const productCatalogue = state.get('designLabLanding').toJS()
...
const DesignLabLandingEnhance = compose(
  injectIntl,
...
export default DesignLabLandingEnhance

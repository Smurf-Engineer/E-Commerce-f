...
if (typeof Float32Array === 'undefined') {
  Filters.getFloat32Array =
    Filters.getUint8Array = function (len: string) {
      if (len.length) {
        return len.slice(0)
      }
  ...
  Filters.getFloat32Array = function (len: number) {
    return new Float32Array(len)
  }
  Filters.getUint8Array = function (len: number) {
...
if (typeof document !== 'undefined') {
  ...
  Filters.getPixels = function (img: any) {
    var c, ctx
    if (img.getContext) {
      c = img
  ...
  Filters.createImageData = function (w: number, h: number) {
    return this.tmpCtx.createImageData(w, h)
  }

  Filters.getCanvas = function (w: number, h: number) {
  ...
  Filters.filterImage = function (filter: any, image: File) {
  ...
  Filters.toCanvas = function (pixels: ImageData) {
    var canvas = this.getCanvas(pixels.width, pixels.height)
    canvas.getContext('2d').putImageData(pixels, 0, 0)
    return canvas
  }

  Filters.toImageData = function (pixels: ImageData) {
...
Filters.runPipeline = function (ds: any) {
  var res = null
...
Filters.createImageDataFloat32 = function (w: number, h: number) {
  return { width: w, height: h, data: this.getFloat32Array(w * h * 4) }
}

Filters.identity = function (pixels: ImageData, args: any) {
...
Filters.horizontalFlip = function (pixels: ImageData) {
  var output = Filters.createImageData(pixels.width, pixels.height)
  var w = pixels.width
  var h = pixels.height
  var dst = output.data
...
Filters.verticalFlip = function (pixels: ImageData) {
  var output = Filters.createImageData(pixels.width, pixels.height)
  var w = pixels.width
  var h = pixels.height
  var dst = output.data
...
Filters.luminance = function (pixels: ImageData, args: any) {
  var output = Filters.createImageData(pixels.width, pixels.height)
  var dst = output.data
  var d = pixels.data
...
Filters.grayscale = function (pixels: ImageData, args: any) {
  var output = Filters.createImageData(pixels.width, pixels.height)
  var dst = output.data
  var d = pixels.data
  for (var i = 0; i < d.length; i += 4) {
...
Filters.grayscaleAvg = function (pixels: ImageData, args: any) {
  var output = Filters.createImageData(pixels.width, pixels.height)
  var dst = output.data
  var d = pixels.data
  var f = 1 / 3
...
Filters.threshold = function (pixels: ImageData, threshold: number, high: number, low: number) {
  var output = Filters.createImageData(pixels.width, pixels.height)
  if (high == null) {
    high = 255
  }
...
Filters.invert = function (pixels: ImageData) {
  var output = Filters.createImageData(pixels.width, pixels.height)
  var d = pixels.data
  var dst = output.data
  for (var i = 0; i < d.length; i += 4) {
...
Filters.brightnessContrast = function (pixels: ImageData, brightness: number, contrast: number) {
...
Filters.applyLUT = function (pixels: ImageData, lut: LUT) {
  var output = Filters.createImageData(pixels.width, pixels.height)
  var d = pixels.data
  var dst = output.data
  var r = lut.r
...
Filters.createLUTFromCurve = function (points: number[][]) {
...
Filters.brightnessContrastLUT = function (brightness: number, contrast: number) {
  var lut = this.getUint8Array(256)
...
Filters.convolve = function (pixels: ImageData, weights: number[], opaque: boolean) {
...
Filters.verticalConvolve = function (pixels: ImageData, weightsVector: number[], opaque: boolean) {
...
Filters.horizontalConvolve = function (pixels: ImageData, weightsVector: number[], opaque: boolean) {
  var side = weightsVector.length
  var halfSide = Math.floor(side / 2)
...
Filters.convolveFloat32 = function (pixels: ImageData, weights: number[], opaque: boolean) {
  var side = Math.round(Math.sqrt(weights.length))
  var halfSide = Math.floor(side / 2)
...
Filters.verticalConvolveFloat32 = function (pixels: ImageData, weightsVector: number[], opaque: boolean) {
  var side = weightsVector.length
  var halfSide = Math.floor(side / 2)
...
Filters.horizontalConvolveFloat32 = function (pixels: ImageData, weightsVector: number[], opaque: boolean) {
  var side = weightsVector.length
  var halfSide = Math.floor(side / 2)
...
Filters.gaussianBlur = function (pixels: ImageData, diameter: number) {
...
Filters.laplaceKernel = Filters.getFloat32Array(
  ...
  -1, 8, -1,
  -1, -1, -1])
Filters.laplace = function (pixels: ImageData) {
...
Filters.sobel = function (px: number) {
...
Filters.bilinearSample = function (pixels: ImageData, x: number, y: number, rgba: number[]) {
  var x1 = Math.floor(x)
  var x2 = Math.ceil(x)
  var y1 = Math.floor(y)
  var y2 = Math.ceil(y)
...
Filters.distortSine = function (pixels: ImageData, amount: number, yamount: number) {
  if (amount == null) {
    amount = 0.5
  }
  if (yamount == null) {
...
Filters.darkenBlend = function (below: ImageData, above: ImageData) {
...
Filters.lightenBlend = function (below: ImageData, above: ImageData) {
  var output = Filters.createImageData(below.width, below.height)
  var a = below.data
  var b = above.data
  var dst = output.data
...
Filters.multiplyBlend = function (below: ImageData, above: ImageData) {
  var output = Filters.createImageData(below.width, below.height)
  var a = below.data
  var b = above.data
  var dst = output.data
...
Filters.screenBlend = function (below: ImageData, above: ImageData) {
  var output = Filters.createImageData(below.width, below.height)
  var a = below.data
  var b = above.data
  var dst = output.data
...
Filters.addBlend = function (below: ImageData, above: ImageData) {
  var output = Filters.createImageData(below.width, below.height)
  var a = below.data
  var b = above.data
  var dst = output.data
...
Filters.subBlend = function (below: ImageData, above: ImageData) {
  var output = Filters.createImageData(below.width, below.height)
  var a = below.data
  var b = above.data
  var dst = output.data
...
Filters.differenceBlend = function (below: ImageData, above: ImageData) {
  var output = Filters.createImageData(below.width, below.height)
  var a = below.data
  var b = above.data
  var dst = output.data
...
Filters.erode = function (pixels: ImageData) {
  var src = pixels.data
  var sw = pixels.width
  var sh = pixels.height
